...
// here we are using the inorder traversal to check my tree is bst or not
template<typename T>
bool check_bst(node<T>*root,int&prev){
    if(root==NULL) return true;

// this line tell me about that left subtree is bst or not
    bool left=check_bst(root->left,prev);
    if(!left) return false;
    // here we have to be in sorted order for the bst
    if(prev>=root->data){
        return false;
    }
    prev=root->data;
    bool right=check_bst(root->right,prev);
    if(!right){
        return false;
    }
    return left&right;
