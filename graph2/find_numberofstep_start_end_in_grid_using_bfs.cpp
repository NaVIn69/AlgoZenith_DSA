...
//here we are trying to find the number of shortest path 
...
void bfs(state st_node){
    ...
    num_paths[st_node.F][st_node.S]=1;
    q.push(st_node);
    while(!q.empty()){
        state node=q.front();
        q.pop();
        for(auto v:neighbour(node)){
             if(!vis[v.F][v.S]){
                ...
                num_paths[v.F][v.S]=num_paths[node.F][node.S];
                par[v.F][v.S]=node;
                q.push(v);
