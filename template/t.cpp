...
// here prefix mex is present 
// prefixmex[i] gives me 0 se ith tak ka mex 
int prefix_mex[100100],suffix_mex[100100];
int arr[100100];
int n;
// 2nd method
void MEX2(){
 ...
 // mex with update the array element
 class Mex {
private:
...
public:
   ...
   //  here prefix mex[i] tells me about the mex 0 se ith tak ka mex
   void MEX(){
    set<int>premex,sufmex;
    int a=0,b=0;
    for(int i=0;i<n;i++){
      premex.insert(arr[i]);
      while(premex.count(b)){
        a++;
      }
      prefix_mex[i]=a;
    }
   ...
   // here we are making the suffix MEX
  for(int i=n-1;i>=0;i--){
    sufmex.insert(arr[i]);
  ...
  ///  struct hasher value for string
  struct hasher{
    int n, _k, _p;
